...
import { PeopleAlt, WindPower, AutoStories, Addchart, LegendToggle, Handyman, ConnectWithoutContact }  from '@mui/icons-material';
...
const MatxVerticalNavExpansionPanel = ({ item, children, mode, level=0 }) => {
    ...
    <NavExpandRoot>
      ...
      >
        <Box display="flex" alignItems="center">
          {icon ? 
            icon === 'PeopleAlt' ? <PeopleAlt sx={{ml: 2}}/> : 
            icon === 'WindPower' ? <WindPower sx={{ml: 2}}/> :
            icon === 'AutoStories' ? <AutoStories sx={{ml: 2}}/> :
            icon === 'Addchart' ? <Addchart sx={{ml: 2}}/> :
            icon === 'Handyman' ? <Handyman sx={{ml: 2}}/> :
            icon === 'ConnectWithoutContact' ? <ConnectWithoutContact sx={{ml: 2}}/> :
            icon === 'LegendToggle' ? <LegendToggle sx={{ml: 2}}/> : <></>
          : <></>}
          {iconText && <BulletIcon />}
      ...
      <div
        ...
        style={collapsed ? { maxHeight: '0px', marginLeft: `${level*8}px` } : { maxHeight: componentHeight.current + 'px', marginLeft: `${level*8}px` }}
      >
        {children}
      </div>
    </NavExpandRoot>
